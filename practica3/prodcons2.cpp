...
#define T_PROD           0
#define T_CONS_TO_BUFF   1
#define T_BUFF_TO_CONS   2
#define BUFFER_ID        5
#define TAM              5
...
void productor(int rank) {
   ...
   for(int i = 0; i < 4; i++) { 
      ...
      MPI_Ssend(&value, 1, MPI_INT, BUFFER_ID, T_PROD, MPI_COMM_WORLD);
   }
}
...
void consumidor(int rank) {
   ...
   for(int i = 0; i < 5; i++) {
      MPI_Ssend(&peticion, 1, MPI_INT, BUFFER_ID, T_CONS_TO_BUFF, MPI_COMM_WORLD); 

      MPI_Recv(&value, 1, MPI_INT, BUFFER_ID, T_BUFF_TO_CONS, MPI_COMM_WORLD, &status);
...
void buffer() {
   ...
   {  
      if      (pos == 0)   rama = 0;        
         ...
         if(status.MPI_TAG == T_PROD) rama = 0; 
         else                         rama = 1;
      }
      ...
      switch(rama) {
      case 0:
         MPI_Recv(&value[pos], 1, MPI_INT, MPI_ANY_SOURCE, T_PROD, MPI_COMM_WORLD, &status );
      ...
      case 1:
         MPI_Recv(&peticion, 1, MPI_INT, MPI_ANY_SOURCE, T_CONS_TO_BUFF, MPI_COMM_WORLD, &status);

         MPI_Ssend(&value[pos-1], 1, MPI_INT, status.MPI_SOURCE, T_BUFF_TO_CONS, MPI_COMM_WORLD);
